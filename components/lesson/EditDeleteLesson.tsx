...
const EditDeleteLesson = ({ route, navigation }: { route: any; navigation: any }) => {
    ...
    useEffect(() => {
        if (currentContent) {
            ...
            setType(currentContent.type || 'VIDEO');
            setDescription(currentContent.description || '');
            setContentUrl(currentContent.contentUrl || '');
            setPoints(String(currentContent.points ?? ''));
            setOrderIndex(String(currentContent.orderIndex ?? ''));
        }
    }, [currentContent]);
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            Alert.alert('Success', 'Learning content updated successfully!', [
                ...
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            ...
                            Alert.alert('Deleted', 'Learning content deleted successfully.', [
                                { text: 'OK', onPress: () => navigation.goBack() },
                            ]);
                        } catch (err: any) {
